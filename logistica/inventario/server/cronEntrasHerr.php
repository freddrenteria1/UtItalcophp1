...
$fecha=date("Y-m-d");
    ...
    $query = "SELECT * FROM entradaherramientasc WHERE remision = ''";
    ...
    while($obj = mysqli_fetch_object($exito)){
        ...
        $remi = 'A'.$lastid;
        $sql2 = "UPDATE entradaherramientasc SET remision = '$remi' WHERE id=$nument";
        ...
        for($i = 0; $i<$canti; $i++){
            ...
            $buscar = "SELECT * FROM invplantaalmc Where codigo = '$cod' And ods='$ods' And almacen = 'Herramientas' And ubicacion = '$ubicacion'";
            ...
            if($cantb > 0){
                ...
                $query2 = "UPDATE invplantaalmc SET cant = $ncant Where codigo = '$cod' And ods='$ods' And almacen = 'Herramientas' And ubicacion = '$ubicacion'";
                $realizar = mysqli_query($conexion, $query2);
            }else{
                $query2 = "INSERT INTO invplantaalmc VALUES('','$fecha', $lastid, '$cod', '$artitem', $cant, '$ods', 'Herramientas', '$ubicacion')";
                $realizar = mysqli_query($conexion, $query2);
                if(!$realizar){
            ...
            $buscar2 = "SELECT * FROM inventarioc Where codigo = '$cod' AND ubicacion = 'AP'";
            $ejeb2 = mysqli_query($conexion, $buscar2);

            $cantb2 = mysqli_num_rows($ejeb2);

            if($cantb2 == 0){
                $query2 = "INSERT INTO inventarioc VALUES('','$tipo', '$codtipo', '$clase', '$codclase', '$articulo', '$cod', '$unidad', 0, 'AP', 'Existente')";
                $realizar = mysqli_query($conexion, $query2);
                if(!$realizar){
                    $msn = mysqli_error($conexion);
                }
