    ...
    function cargarDatos(semb){
        $.post(server+'listasOM.php', {semana:numsem},
            function(resp){
                console.log(resp)
                arrayDatos = resp.om
                arrayEmergentes = resp.datosemer
                arrayEmerDet = resp.datosemerdet
                ...
                html += `
                <button class="btn btn-dark" onclick="cargarEmer()">EMERGENTES</button>
        ...
        function cargarEmer(){
            if(arrayEmergentes == null){
                Swal.fire({
                    position: 'top-end',
                    icon: 'info',
                    title: 'Importante!..',
                    text: 'Sin emergentes en la semana',
                    showConfirmButton: false,
                    timer: 2500
                })
            }else{
                cargarGraficaEmergentes()
            }
        }
        ...
        function cargarGrafica(){
            ...
            for(var i=0; i<arrayDatos.length; i++){
                ...
                pdte = arrayDatos[i].cantom - arrayDatos[i].cantomeje
                listapdte.push(pdte)
                cantpdtet += pdte
            }
        ...
        function cargarGraficaEmergentes(){
            ...
            for(var i=0; i<arrayEmergentes.length; i++){
                listaFrentes.push(arrayEmergentes[i].frente)
                listaprog.push(arrayEmergentes[i].cantom)
                cantprog += arrayEmergentes[i].cantom;
                listaeje.push(arrayEmergentes[i].cantom)
                canteje += arrayEmergentes[i].cantom
                pdte = arrayEmergentes[i].cantom - arrayEmergentes[i].cantom
                listapdte.push(pdte)
                cantpdtet += pdte
            }
            ...
            var html = `<br><br>
            <table class="table table-striped"><thead><th>OM</th><th>OP</th><th>Equipo</th><th>Actividad</th>
            <th>Frente</th><th>Finicio</th><th>Estado</th></thead><tbody>
            `
            for(var i=0; i<arrayEmerDet.length; i++){
                html += `
                <tr>
                    <td>${arrayEmerDet[i].orden}</td>
                    <td>${arrayEmerDet[i].op}</td>
                    <td>${arrayEmerDet[i].equipo}</td>
                    <td>${arrayEmerDet[i].actividad}</td>
                    <td>${arrayEmerDet[i].frente}</td>
                    <td>${arrayEmerDet[i].finicio}</td>
                    <td>${arrayEmerDet[i].estado}</td>
                </tr>
                `
            }
            html += `
        ...
        function cargarGraficaFrente(){
            ...
            for(var i=0; i<arrayDatosFrentes.length; i++){
                ...
                pdte = arrayDatosFrentes[i].plan - arrayDatosFrentes[i].eje
                totpdte += pdte
                listapdte.push(pdte)
