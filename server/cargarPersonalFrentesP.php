...
$frente = str_replace('022 ', '', $ods);

$sql="SELECT turno, SUM(directo) as totdir, SUM(indirecto) as totindir, SUM(total) as totp, SUM(hdir) as hdir, SUM(hindir) as hindir, SUM(thh) as tothh FROM turnosdiarios WHERE  frente like '%$frente%' AND fecha = '$fecha' GROUP BY turno";
...
$query = "SELECT SUM(hdir) as totdir, SUM(hindir) as totindir, SUM(thh) as tothh From turnosdiarios Where frente like '%$frente%'";
...
$query3 = "SELECT SUM(total) as totp From turnosdiarios Where frente like '%$frente%'";
