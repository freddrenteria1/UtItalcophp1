...
$totConv = $totalPersonal * 0.25;
    ...
    $sqlh = "SELECT frente, SUM(directo) as directos, SUM(indirecto) as indirectos, SUM(total) as total FROM turnosdiarios Where fecha='$fecha' and ods = '$ods' GROUP BY frente";
    ...
    $sqlh = "SELECT frente, SUM(hdir) as hdir, SUM(hindir) as hindir, SUM(thh) as thh FROM turnosdiarios Where fecha='$fecha' and ods = '$ods'  AND fecha <='$fecha' GROUP BY frente";
    $ejeh = mysqli_query($conexion, $sqlh);
    
    while($filah = mysqli_fetch_object($ejeh)){
        
        $personalfrenteshoras[] = array(
            'frente'=>$filah->frente,
    ...
    $sqlh = "SELECT frente, SUM(hdir) as hdir, SUM(hindir) as hindir, SUM(thh) as thh FROM turnosdiarios Where ods = '$ods' AND frente = 'PREFABRICADO EXTERNO' AND fecha <='$fecha'";
    $ejeh = mysqli_query($conexion, $sqlh);
