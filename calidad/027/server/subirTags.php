    ...
    while (($fila = fgetcsv($gestor, $longitudDeLinea, $delimitador, $caracterCircundante)) !== false) {
        ...
        if ($numeroDeFila > 1) {
            ...
            if($fila[0] != ''){
                ...
                // $busqueda = "SELECT * FROM tags WHERE ods='027' AND lazo='$fila[0]' AND tag = '$fila[4]'";
                // $ejeb = mysqli_query($conexion, $busqueda);
                // $cont = mysqli_num_rows($ejeb);

                // if($cont == 0){
                //     $query = "INSERT INTO tags VALUES('', '027', 'U-2700', 'UOP I', 'TUBERÍA', 'PUNTOS DE INSPECCIÓN', 'ALISTAMIENTO', '$fila[0]', '$fila[4]', 'OS114')";
                ...
                $busqueda2 = "SELECT * FROM os114 WHERE ods='027' AND isometrico = '$fila[4]' AND lazo_corrosion ='$fila[0]'";
                $ejeb2 = mysqli_query($conexion, $busqueda2);
                $cont2 = mysqli_num_rows($ejeb2);

                if($cont2 == 0){
                    $query2 = "INSERT INTO os114 VALUES('', 'U-650', '', '', '027', '$fila[4]', '$fila[0]', '$cmls', '', '', '', '', '')";
                    $eje = mysqli_query($conexion, $query2);
                }else{
                    $query2 = "UPDATE os114 SET cml_inspeccionados = '$cmls' WHERE ods='027' AND lazo_corrosion = '$fila[0]' AND isometrico = '$fila[4]'";
                    $eje = mysqli_query($conexion, $query2);
                }
