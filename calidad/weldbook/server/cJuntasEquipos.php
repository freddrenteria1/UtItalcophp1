    ...
    while( $obj = mysqli_fetch_object($exito)){
        ...
        while($obje = mysqli_fetch_object($exitoe)){
            ...
            while($objp = mysqli_fetch_object($exitop)){
                ...
                if($sumjpt != 0){
                    $ajt = ($sumjet/$sumjpt)*100 . '%';
                }else{
                    $ajt = '-';
                }

                if($sumjpt != 0){
                    $apt = ($sumpet/$sumppt)*100 . '%';
                }else{
                    $apt = '-';
                }
                ...
                if($sumjpc != 0){
                    $ajc = ($sumjec/$sumjpc)*100 . '%';
                }else{
                    $ajc = '-';
                }

                if($sumjpc != 0){
                    $apc = ($sumpec/$sumppc)*100 . '%';
                }else{
                    $apc = '-';
                }

                $jptot = floatval($objp->juntasp);
                $pptot = floatval($objp->pulgadasp);

                $jetot = $sumjet + $sumjec;
                $petot = $sumpet + $sumpec;

                $ajtot = ($jetot/$jptot)*100 . '%';
                $aptot = ($petot/$pptot)*100 . '%';
