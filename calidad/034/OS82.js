...
function cargarDatos() {
	...
	if (arrayDatos.checklist != "") {
		arrayItems = JSON.parse(arrayDatos.checklist)
		for (var i = 0; i <= 4; i++) {
			b = i + 1;
	...
	if (arrayDatos.firmachecklist != "") {
		var arrayFirmasItems = JSON.parse(arrayDatos.firmachecklist)
	...
	if (arrayDatos.docchecklist != "") {
		listaArchivos = JSON.parse(arrayDatos.docchecklist)
	}
	...
	if (arrayDatos.firmascal != "") {
		var arrayFirmasItems = JSON.parse(arrayDatos.firmascal)
	...
	if (arrayDatos.obscal != "") {
		var arrayObs2 = JSON.parse(arrayDatos.obscal)
		var html = ''
		for (var i = 0; i < arrayObs2.length; i++) {
			html = `<b>Nota ${i + 1}: </b>
                    ${arrayObs2[i].obs} | ${arrayObs2[i].nombre} | Registro: ${arrayObs2[i].registro} | Fecha: ${arrayObs2[i].fecha} | Firma: <span id="firmcal${i}" style="width: 60px;"></span> <br>--------------------------------------------------------------------------------------------------------------------------------------------------------------------<br>
                    `
			$('#observaciones2').append(html)
			const imagef = document.createElement('img')
			imagef.src = arrayObs2[i].firma
			imagef.height = "60"
...
function actFirmasCheck() {
	...
	$.post(server + 'act82actFirmasCheck.php', {
		ods: odsq,
		tag: tag,
...
function actEstadoInicial() {
	...
	$.post(server + 'act82FirmaEstInicial.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayFirmasItems)
	},
...
function actEstadoFinal() {
	...
	$.post(server + 'act82FirmaEstFinal.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayFirmasItems)
	},
...
function adjuntar() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: server + 'guardarArchivo82.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
...
function adjuntar2() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			formData.append('tag', tag);
			$.ajax({
				url: server + 'guardarArchivo82Cal.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
