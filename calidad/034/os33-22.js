...
function actAlistamiento() {
	...
	$.post(server + 'act3322alistamiento.php', {
		ods: odsq,
		tag: tag,
...
function actLiberacion() {
	...
	$.post(server + 'act3322Liberacion.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function actComp() {
	...
	$.post(server + 'ac3322Componentes.php', {
		ods: odsq,
		tag: tag,
...
function adjuntar() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: server + 'guardarArchivo3322.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
