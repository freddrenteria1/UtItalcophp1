...
function actAlistamiento() {
	...
	$.post(server + 'act3325alistamiento.php', {
		ods: odsq,
		tag: tag,
...
function actLiberacion() {
	...
	$.post(server + 'act3325Liberacion.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function actComp() {
	...
	$.post(server + 'ac3325Componentes.php', {
		ods: odsq,
		tag: tag,
...
function adjuntar() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: server + 'guardarArchivo3325.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
