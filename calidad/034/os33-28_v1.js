...
function pasarFirma() {
	if ($('#poldatos').prop('checked')) {
		...
		if (idImagen >= 5 && idImagen <= 28) {
			actComp()
		}
		if (idImagen >= 29 && idImagen <= 31) {
			actPintura()
		}
...
function cargarDatos() {
	...
	if (arrayDatos.liberacion != "") {
		...
		$('#itemLib4').val(arrayInfo[0].itemLib4)
	...
	if (arrayDatos.pintura != "") {
		...
		arrayInfo = JSON.parse(arrayDatos.pintura)


		$('#itemPint1').val(arrayInfo[0].itemPint1)
		$('#itemPint2').val(arrayInfo[0].itemPint2)
		 

		$('#fecha29').val(arrayInfo[0].fechautsup)
		$('#hora29').val(arrayInfo[0].horautsup)
		$('#nombre29').val(arrayInfo[0].nombreutsup)

		if (arrayInfo[0].fechautsup != "") {
			document.querySelector("#firma29").src = arrayInfo[0].firmautsup;
			$('#firma29').removeClass('oculto')
			$('#btnFirmar29').hide()
		}

		$('#registro29').val(arrayInfo[0].registroutsup)

		$('#fecha30').val(arrayInfo[0].fechautqaqc)
		$('#hora30').val(arrayInfo[0].horautqaqc)
		$('#nombre30').val(arrayInfo[0].nombreutqaqc)

		if (arrayInfo[0].fechautqaqc != "") {
			document.querySelector("#firma30").src = arrayInfo[0].firmautqaqc;
			$('#firma30').removeClass('oculto')
			$('#btnFirmar30').hide()
		}

		$('#registro30').val(arrayInfo[0].registroecp)

		$('#fecha31').val(arrayInfo[0].fechaecp)
		$('#hora31').val(arrayInfo[0].horaecp)
		$('#nombre31').val(arrayInfo[0].nombreecp)

		if (arrayInfo[0].fechaecp != "") {
			document.querySelector("#firma31").src = arrayInfo[0].firmaecp;
			$('#firma31').removeClass('oculto')
			$('#btnFirmar31').hide()
		}

		$('#registro31').val(arrayInfo[0].registroecp)

		
	}

	if (arrayDatos.auditoria != "") {

		arrayInfo = JSON.parse(arrayDatos.auditoria)
		...
		if (arrayInfo[0].fechaecp != "") {
			document.querySelector("#firma32").src = arrayInfo[0].firmaecp;
...
function actAlistamiento() {
	...
	$.post(server + 'act3328alistamiento.php', {
		ods: odsq,
		tag: tag,
...
function actLiberacion() {
	...
	$.post(server + 'act3328Liberacion.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function actComp() {
	...
	$.post(server + 'ac3328Componentes.php', {
		ods: odsq,
		tag: tag,
...
function actPintura() {
	var itemPint1 = $('#itemPint1').val()
	var itemPint2 = $('#itemPint2').val()
	 

	var fechautsup = $('#fecha29').val()
	var horautsup = $('#hora29').val()
	var nombreutsup = $('#nombre29').val()

	var img = document.getElementById("firma29");
	var firmautsup = img.src
	...
	var registroutsup = $('#registro29').val()

	var fechautqaqc = $('#fecha30').val()
	var horautqaqc = $('#hora30').val()
	var nombreutqaqc = $('#nombre30').val()

	var img = document.getElementById("firma30");
	var firmautqaqc = img.src
	...
	var registroutqaqc = $('#registro30').val()


	var fechaecp = $('#fecha31').val()
	var horaecp = $('#hora31').val()
	var nombreecp = $('#nombre31').val()

	var img = document.getElementById("firma31");
	...
	var registroecp = $('#registro31').val()
	...
	$.post(server + 'act3328pintura.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function actAudi() {
	...
	$.post(server + 'act3328Auditoria.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function adjuntar() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: server + 'guardarArchivo3328.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
