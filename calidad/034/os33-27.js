...
function actAlistamiento() {
	...
	$.post(server + 'act3327alistamiento.php', {
		ods: odsq,
		tag: tag,
...
function actLiberacion() {
	...
	$.post(server + 'act3327Liberacion.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function actComp() {
	...
	$.post(server + 'ac3327Componentes.php', {
		ods: odsq,
		tag: tag,
...
function adjuntar() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: server + 'guardarArchivo3327.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
