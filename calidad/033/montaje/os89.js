...
function cargarDatos() {
    ...
    if (arrayDatos.itemvalvulas != "") {
        arrayItems = JSON.parse(arrayDatos.itemvalvulas)
        for (var i = 0; i <= 25; i++) {
            b = i + 1;
    ...
    if (arrayDatos.datos != "") {
        var arrayFirmasItems = JSON.parse(arrayDatos.datos)
    ...
    if (arrayDatos.datoscal != "") {
        arrayItems = JSON.parse(arrayDatos.datoscal)
        for (var i = 0; i <= 4; i++) {
            b = i + 1;
            $('#dcs' + b).val(arrayItems[i].dcs)
            $('#apertura' + b).val(arrayItems[i].apertura)
            $('#lectura' + b).val(arrayItems[i].lectura)
        }
    }

    if (arrayDatos.equipos != "") {
        arrayItems = JSON.parse(arrayDatos.equipos)
        for (var i = 0; i <= 1; i++) {
            b = i + 1;
            $('#marcaEU' + b).val(arrayItems[i].marcaEU)
            $('#modeloEU' + b).val(arrayItems[i].modeloEU)
            $('#serieEU' + b).val(arrayItems[i].serieEU)
            $('#fechaEU' + b).val(arrayItems[i].fechaEU)
        }
    }

    if (arrayDatos.firmasvalvulas != "") {
        var arrayFirmasItems = JSON.parse(arrayDatos.firmasvalvulas)
    ...
    if (arrayDatos.estadofinal != "") {
        ...
        $('#fecha8').val(arrayFirmasItems[0].fechaec)
        $('#hora8').val(arrayFirmasItems[0].horaec)
        $('#nombre8').val(arrayFirmasItems[0].nombreec)

        if (arrayFirmasItems[0].fechaec != "") {
            document.querySelector("#firma8").src = arrayFirmasItems[0].firmaec;
            $('#firma8').removeClass('oculto')
            $('#btnFirmar8').hide()
        }

        $('#registro8').val(arrayFirmasItems[0].registroec)

        //ut

        $('#fecha7').val(arrayFirmasItems[0].fechaut)
        $('#hora7').val(arrayFirmasItems[0].horaut)
        $('#nombre7').val(arrayFirmasItems[0].nombreut)

        if (arrayFirmasItems[0].fechaut != "") {
            document.querySelector("#firma7").src = arrayFirmasItems[0].firmaut;
            $('#firma7').removeClass('oculto')
            $('#btnFirmar7').hide()
        }

        $('#registro7').val(arrayFirmasItems[0].registrout)
    ...
    if (arrayDatos.obslazo != "") {
        var arrayObs = JSON.parse(arrayDatos.obslazo)
        var html = ''
    ...
    if (arrayDatos.obsvalvulas != "") {
        var arrayObs = JSON.parse(arrayDatos.obsvalvulas)
        var html = ''
        for (var i = 0; i < arrayObs.length; i++) {
            html = `<b>Nota ${i + 1}: </b>
                    ${arrayObs[i].obs} | ${arrayObs[i].nombre} | Registro: ${arrayObs[i].registro} | Fecha: ${arrayObs[i].fecha} | Firma: <span id="firmv${i}" style="width: 60px;"></span> <br>--------------------------------------------------------------------------------------------------------------------------------------------------------------------<br>
                    `
            $('#observaciones2').append(html)
            const imagef = document.createElement('img')
            imagef.src = arrayObs[i].firma
            imagef.height = "60"
            document.querySelector('#firmv' + i).appendChild(imagef)
    ...
    if (arrayDatos.doclazo != "") {
        listaArchivos = JSON.parse(arrayDatos.doclazo)

    }

    if (arrayDatos.docvalvulas != "") {
        listaArchivosV = JSON.parse(arrayDatos.docvalvulas)
...
function actDatosPrueba() {
    ...
    $.post(server + 'act89FirmaDatosPrueba.php', {
        ods: odsq,
        tag: tag,
...
function actEstadoInicial() {
    ...
    $.post(server + 'act89FirmaEstInicial.php', {
        ods: odsq,
        tag: tag,
        datos: JSON.stringify(arrayFirmasItems)
    },
...
function actEstadoFinal() {
    ...
    var fechaec = $('#fecha8').val()
    var horaec = $('#hora8').val()
    var nombreec = $('#nombre8').val()
    ...
    var registroec = $('#registro8').val()

    var fechaut = $('#fecha7').val()
    var horaut = $('#hora7').val()
    var nombreut = $('#nombre7').val()
    ...
    $.post(server + 'act89FirmaEstFinal.php', {
        ods: odsq,
        tag: tag,
        datos: JSON.stringify(arrayFirmasItems)
    },
...
function verArchivos2() {
    ...
    for (var i = 0; i < listaArchivosV.length; i++) {
        htmltexto += `
                    <a href='https://utitalco.com/calidad/033/montaje/server/archivos/${listaArchivosV[i].archivo}' target='_blank'>${listaArchivosV[i].archivo}</a> <br>
                `
    }
...
function adjuntar() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: server + 'guardarArchivo89Lz.php',
                type: 'post',
                data: formData,
                contentType: false,
                processData: false,
...
function adjuntar2() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            formData.append('tag', tag);
            $.ajax({
                url: server + 'guardarArchivo89Val.php',
                type: 'post',
                data: formData,
                contentType: false,
                processData: false,
...
function guardarObs() {
    ...
    if (arrayDatos.obslazo != "") {
        ...
        var arrayLmp = JSON.parse(arrayDatos.obslazo)
    } else {
        var arrayLmp = []
    }
...
function guardarObs2() {
    ...
    if (arrayDatos.obsvalvulas != "") {
        ...
        var arrayLmp = JSON.parse(arrayDatos.obsvalvulas)
    } else {
        var arrayLmp = []
    }
