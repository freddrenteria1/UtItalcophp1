...
function pasarFirma() {
	if ($('#poldatos').prop('checked')) {
		...
		if (idImagen >= 3 && idImagen <= 6) {
			actLimpieza()
		}

		if (idImagen >= 7 && idImagen <= 33) {
			actComp()
		}
		if (idImagen >= 34 && idImagen <= 35) {
			actLiberacionTrab()
		}
...
function cargarDatos() {
	...
	if (arrayDatos.limpieza != "") {
		...
		arrayInfo = JSON.parse(arrayDatos.limpieza)


		$('#itemLimp1').val(arrayInfo[0].itemLimp1)
		$('#itemLimp2').val(arrayInfo[0].itemLimp2)
		...
		$('#fecha5').val(arrayInfo[0].fechautsup2)
		$('#hora5').val(arrayInfo[0].horautsup2)
		$('#nombre5').val(arrayInfo[0].nombreutsup2)

		if (arrayInfo[0].fechautsup2 != "") {
			document.querySelector("#firma5").src = arrayInfo[0].firmautsup2;
			$('#firma5').removeClass('oculto')
			$('#btnFirmar5').hide()
		}

		$('#registro5').val(arrayInfo[0].registroutsup2)

		$('#fecha6').val(arrayInfo[0].fechautqaqc2)
		$('#hora6').val(arrayInfo[0].horautqaqc2)
		$('#nombre6').val(arrayInfo[0].nombreutqaqc2)

		if (arrayInfo[0].fechautqaqc2 != "") {
			document.querySelector("#firma6").src = arrayInfo[0].firmautqaqc2;
			$('#firma6').removeClass('oculto')
			$('#btnFirmar6').hide()
		}

		$('#registro6').val(arrayInfo[0].registroutqaqc2)
	...
	if (arrayDatos.libtrab != "") {

		arrayInfo = JSON.parse(arrayDatos.libtrab)


		$('#itemLibT1').val(arrayInfo[0].itemLibT1)
		$('#itemLibT2').val(arrayInfo[0].itemLibT2)
		$('#itemLibT3').val(arrayInfo[0].itemLibT3)

		$('#numrtT').val(arrayInfo[0].numrtT)
		 

		

		$('#fecha34').val(arrayInfo[0].fechautqaqc)
		$('#hora34').val(arrayInfo[0].horautqaqc)
		$('#nombre34').val(arrayInfo[0].nombreutqaqc)

		if (arrayInfo[0].fechautqaqc != "") {
			document.querySelector("#firma34").src = arrayInfo[0].firmautqaqc;
		...
		$('#fecha35').val(arrayInfo[0].fechaecp)
		$('#hora35').val(arrayInfo[0].horaecp)
		$('#nombre35').val(arrayInfo[0].nombreecp)

		if (arrayInfo[0].fechaecp != "") {
			document.querySelector("#firma35").src = arrayInfo[0].firmaecp;
			$('#firma35').removeClass('oculto')
			$('#btnFirmar35').hide()
		}

		$('#registro35').val(arrayInfo[0].registroecp)
...
function actAlistamiento() {
	...
	$.post(server + 'act3329alistamiento.php', {
		ods: odsq,
		tag: tag,
...
function actLimpieza() {
	var itemLimp1 = $('#itemLimp1').val()
	var itemLimp2 = $('#itemLimp2').val()
	...
	var fechautsup2 = $('#fecha5').val()
	var horautsup2 = $('#hora5').val()
	var nombreutsup2 = $('#nombre5').val()
	...
	var registroutsup2 = $('#registro5').val()

	var fechautqaqc2 = $('#fecha6').val()
	var horautqaqc2 = $('#hora6').val()
	var nombreutqaqc2 = $('#nombre6').val()

	var img = document.getElementById("firma6");
	var firmautqaqc2 = img.src

	if (fechautqaqc2 == '') {
		firmautqaqc2 = ''
	}

	var registroutqaqc2 = $('#registro6').val()
	...
	arrayInfo.push({
		...
		'fechautqaqc2': fechautqaqc2,
		'horautqaqc2': horautqaqc2,
		'nombreutqaqc2': nombreutqaqc2,
		'firmautqaqc2': firmautqaqc2,
		'registroutqaqc2': registroutqaqc2
	})

	$.post(server + 'act3329Limpieza.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function actComp() {
	...
	$.post(server + 'ac3329Componentes.php', {
		ods: odsq,
		tag: tag,
...
function actLiberacionTrab() {
	var itemLibT1 = $('#itemLibT1').val()
	var itemLibT2 = $('#itemLibT2').val()
	var itemLibT3 = $('#itemLibT3').val()

	var numrtT = $('#numrtT').val()
	
	 

	var fechautqaqc = $('#fecha34').val()
	var horautqaqc = $('#hora34').val()
	var nombreutqaqc = $('#nombre34').val()

	var img = document.getElementById("firma34");
	var firmautqaqc = img.src
	...
	var registroutqaqc = $('#registro34').val()


	var fechaecp = $('#fecha35').val()
	var horaecp = $('#hora35').val()
	var nombreecp = $('#nombre35').val()

	var img = document.getElementById("firma35");
	...
	var registroecp = $('#registro35').val()
	...
	$.post(server + 'act3329LiberacionTrab.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function adjuntar() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: server + 'guardarArchivo3329.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
