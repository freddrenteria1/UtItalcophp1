...
function cargarDatos() {
    ...
    if (arrayDatos.info != "") {
        ...
        $('#tiposwictche').val(arrayInfo[0].tiposwictche)
    ...
    if (arrayDatos.datoscalibracion != "") {
        arrayItems = JSON.parse(arrayDatos.datoscalibracion)
        for (var i = 0; i <= 5; i++) {
            b = i + 1;
            $('#subiendoa' + b).val(arrayItems[i].subiendoa)
            $('#bajandoa' + b).val(arrayItems[i].bajandoa)
            $('#subiendob' + b).val(arrayItems[i].subiendob)
            $('#bajandob' + b).val(arrayItems[i].bajandob)
            $('#bandaM' + b).val(arrayItems[i].bandaM)
        }
    }
    ...
    $('#conexion').val(arrayDatos.conexion)
...
function actInfo() {
    var serie = $('#serie').val()
    var fecha = $('#fecha').val()
    var tiposwictche = $('#tiposwictche').val()
    var marca = $('#marca').val()
...
function actConexion() {
    var conexion = $('#conexion').val()

    $.post(server + 'act112Conexion.php', {
        ods: odsq,
        tag: tag,
        conexion: conexion
    },
...
function actDatos() {
    ...
    for (var i = 1; i <= 6; i++) {
        var subiendoa = $('#subiendoa' + i).val()
        var bajandoa = $('#bajandoa' + i).val()
        var subiendob = $('#subiendob' + i).val()
        var bajandob = $('#bajandob' + i).val()
        var bandaM = $('#bandaM' + i).val()
...
function actEstadoInicial() {
    ...
    $.post(server + 'act112FirmaEstInicial.php', {
        ods: odsq,
        tag: tag,
        datos: JSON.stringify(arrayFirmasItems)
    },
...
function actEstadoFinal() {
    ...
    $.post(server + 'act112FirmaEstFinal.php', {
        ods: odsq,
        tag: tag,
        datos: JSON.stringify(arrayFirmasItems)
    },
...
function adjuntar() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: server + 'guardarArchivo112.php',
                type: 'post',
                data: formData,
                contentType: false,
                processData: false,
