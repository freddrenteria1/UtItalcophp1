...
function actAlistamiento() {
	...
	$.post(server + 'act3323alistamiento.php', {
		ods: odsq,
		tag: tag,
...
function actLiberacion() {
	...
	$.post(server + 'act3323Liberacion.php', {
		ods: odsq,
		tag: tag,
		datos: JSON.stringify(arrayInfo)
	},
...
function actComp() {
	...
	$.post(server + 'ac3323Componentes.php', {
		ods: odsq,
		tag: tag,
...
function adjuntar() {
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: server + 'guardarArchivo3323.php',
				type: 'post',
				data: formData,
				contentType: false,
				processData: false,
