...
$sql="SELECT * FROM tags WHERE ods='$ods' GROUP BY esp";
...
while($obj = mysqli_fetch_object($exito)){
    ...
    $buscar = "SELECT * FROM tags Where esp = '$esp' GROUP BY fequipo";
    ...
    if($encb != 0){
        ...
        while($row = mysqli_fetch_object($ejeb)){
            ...
            $bus = "SELECT * FROM tags Where esp = '$esp' AND fequipo = '$fequipo' group by tag";
            $ejet = mysqli_query($conexion, $bus);

            $canttag = mysqli_num_rows($ejet);

            $busr = "SELECT * FROM tags Where esp = '$esp' AND fequipo = '$fequipo'";
