...
function cargarDatos() {
    ...
    if (arrayDatos.alistamiento != "") {
        ...
        var arrayAlistamiento = JSON.parse(arrayDatos.alistamiento)

        $('#al1').val(arrayAlistamiento[0].al1)
        $('#al2').val(arrayAlistamiento[0].al2)
        $('#al3').val(arrayAlistamiento[0].al3)

        $('#fecha1').val(arrayAlistamiento[0].fechautsup)
        $('#hora1').val(arrayAlistamiento[0].horautsup)
        $('#nombre1').val(arrayAlistamiento[0].nombreutsup)

        document.querySelector("#firma1").src = arrayAlistamiento[0].firmautsup;

        if (arrayAlistamiento[0].fechautsup != "") {
            $('#firma1').removeClass('oculto')
            $('#btnFirmar1').hide()
        }
        $('#registro1').val(arrayAlistamiento[0].registroutsup)

        $('#fecha2').val(arrayAlistamiento[0].fechautqaqc)
        $('#hora2').val(arrayAlistamiento[0].horautqaqc)
        $('#nombre2').val(arrayAlistamiento[0].nombreutqaqc)

        document.querySelector("#firma2").src = arrayAlistamiento[0].firmautqaqc;

        if (arrayAlistamiento[0].fechautqaqc != "") {
            $('#firma2').removeClass('oculto')
            $('#btnFirmar2').hide()
        }

        $('#registro2').val(arrayAlistamiento[0].registroutqaqc)

    }

    if (arrayDatos.libItems != "") {

        var arrayInfo = JSON.parse(arrayDatos.libItems)

        $('#itemLibBanco1').val(arrayInfo[0].itemLibBanco1)
        $('#itemLibBanco2').val(arrayInfo[0].itemLibBanco2)
        $('#itemLibBanco3').val(arrayInfo[0].itemLibBanco3)
        $('#comp1').val(arrayInfo[0].comp1)
    ...
    //cargue de datos de componentes

    if (arrayDatos.libComp != "") {

        var num = 4;
        var numc = 1;

        var arrayInfo = JSON.parse(arrayDatos.libComp)
        ...
        for (var i = 0; i <= 8; i++) {
            ...
            document.querySelector("#firma" + num).src = arrayInfo[i].firmautqaqc;
            ...
            document.querySelector("#firma" + num).src = arrayInfo[i].firmaec;
...
function actAlistamiento() {
    ...
    $.post(server + 'actOs3316Alistamiento.php', {
        ods: odsq,
        tag: tag,
...
function actLibBanco() {

    itemLibBanco1 = $('#itemLibBanco1').val()
    itemLibBanco2 = $('#itemLibBanco2').val()
    itemLibBanco3 = $('#itemLibBanco3').val()
    comp1 = $('#comp1').val()
    numrt = $('#numrt').val()
    ...
    arrayAlist.push({
        "itemLibBanco1": itemLibBanco1,
        "itemLibBanco2": itemLibBanco2,
        "itemLibBanco3": itemLibBanco3,
        "comp1": comp1,
        "numrt": numrt,
        "fechautsup": fechautsup,
        "horautsup": horautsup,
    ...
    $.post(server + 'actOs3316LibBanco.php', {
        ods: odsq,
        tag: tag,
        datos: JSON.stringify(arrayAlist)
    },
...
function actCompBanco() {
    ...
    $.post(server + 'act33316BancoComp.php', {
        ods: odsq,
        tag: tag,
...
function adjuntar() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: server + 'guardarArchivo3316.php',
                type: 'post',
                data: formData,
                contentType: false,
                processData: false,
