...
while($obj = mysqli_fetch_object($ejec)){
    ...
    if($obj->limpeza_interior != ""){
        $limpeza_interior =  json_decode($obj->limpeza_interior);

        $cant = COUNT($limpeza_interior);

        for($i=0; $i<$cant;$i++){
            if($limpeza_interior[$i]->nombreut != ""){
                $sumfirmaut++;
            }
    ...
    if($obj->ejecucion_interior != ""){
        ...
        if($ejecucion_interior[0]->nombreec != ""){
            $sumfirmaec++;
        }
    }
...
$totalequipos = $cantintercambiadores + $canttorres + $cantlgs + $cantvalvulas + $canttambores;
$totalitalcoplan = $totalinterut + $totaltorresut + $totallgut + $totalvalut + $totaltamborut;
$totalecoplan = $totalinterecp + $totaltorresec +  $totallgec + $totalvalec + $totaltamborec;
$totalitalcoeje = $totalinteruteje + $totaltorresuteje + $totallguteje + $totalvaluteje + $totaltamboreceje;
$totalecoeje = $totalintereceje + $totalvaleceje + $totallgeceje + $totaltorreseceje + $totaltamboreceje;
