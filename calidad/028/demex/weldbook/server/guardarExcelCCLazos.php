...
while (($fila = fgetcsv($gestor, $longitudDeLinea, $delimitador, $caracterCircundante)) !== false) {
    ...
    if ($numeroDeFila > 7) {
        ...
        if($contb == 0){
            $sql = "DELETE FROM cclazos";
            $exit = mysqli_query($conexion, $sql);
            
            $contb++;
        } 

        $actividad = trim($fila[8]);


        if($fila[1] != ''){

            $query = "INSERT INTO cclazos VALUES('', '$fila[0]', '$fila[1]', '$fila[2]', '$fila[3]', '$fila[4]', '$fila[5]', '$fila[6]', '$fila[7]', '$actividad', '$fila[9]', '$fila[10]', '$fila[11]', '$fila[12]', '$fila[13]', '$fila[14]', '$fila[15]', '$fila[16]', '$fila[17]', '$fila[18]', '$fila[19]', '$fila[20]', '$fila[21]', '$fila[22]', '$fila[23]', '$fila[24]', '$fila[25]', '$fila[26]')";
...
$sql = "INSERT INTO bitacoracclazos VALUES('', '$user', '$fecha', '$excel')";
