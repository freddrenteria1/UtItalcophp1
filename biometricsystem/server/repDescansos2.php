...
while($obj = mysqli_fetch_object($ejeq)){
    ...
    if($fechainicio != '0000-00-00' && $fechainicio > '2022-01-01'){

        $color = '';

        $firstDate  = new DateTime($fechainicio);
        $secondDate = new DateTime($fecha);
    
        $intvl = $firstDate->diff($secondDate);
    
        $cantdias = $intvl->days;
        $fechabuscar = $fechainicio;
        $contador = 0;
        ...
        for($i=0; $i<$cantdias; $i++){
            ...
            $cons = "SELECT * FROM marcaciones Where fecha = '$fechabuscar' AND doc='$idtrab'";
