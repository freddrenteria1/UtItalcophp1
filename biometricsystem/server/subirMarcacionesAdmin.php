...
if (!$gestor) {
    ...
    $borrar = "DELETE FROM marcaciones Where fecha='$fecha' and terminal != 'Manual' and terminal != 'Anulada'";
    ...
    while (($fila = fgetcsv($gestor, $longitudDeLinea, $delimitador, $caracterCircundante)) !== false) {
        ...
        if ($numeroDeFila > 1) {
            ...
            if($fila[0] != ''){
                ...
                $sql = "SELECT * FROM trabajadores Where id=$idtrab AND estado = 'Activo'";
                ...
                if($tipom == 'Check In' OR $tipom == 'Entrada'){
                    $tipo = 'Entrada';
                }

                if($tipom == 'Check Out' OR $tipom == 'Salida'){
                    $tipo = 'Salida';
                }
                ...
                $query = "INSERT INTO marcaciones VALUES('', '$fila[0]', '$fila[1]','$fecha','$hora','$tipo','$fila[11]', '$turno', '$ods')";
