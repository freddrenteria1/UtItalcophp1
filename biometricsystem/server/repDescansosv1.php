...
$fechainicial = $fecha;
$fechafinal = date("Y-m-d",strtotime($fechabuscar."- 6 days"));


$cons = "SELECT * FROM marcaciones Where fecha  between '$fechafinal' AND '$fechainicial' AND tipo='Entrada' GROUP BY tipo ";
...
while($obj = mysqli_fetch_object($ejeq)){
    ...
    $cantFechas = count($arrayFechas);

    for($i=0; $i <=  $cantFechas; $i++){
        // echo 'Id en fecha: ' .  $arrayFechas[$i]["doc"] . ' Id trab:  ' . $idtrab;
        // echo  '<br>';

        if($idtrab == $arrayFechas[$i]["doc"]){
            $contador++;
        }
    }
