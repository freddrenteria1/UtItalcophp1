...
if($ods != "0"){
    $query = "SELECT * FROM trabajadores Where ods='$ods' order by nombres";
    $eje = mysqli_query($conexion, $query);
}else{
    $query = "SELECT * FROM trabajadores order by nombres";
    $eje = mysqli_query($conexion, $query);
}
...
while($obj = mysqli_fetch_object($eje)){
    ...
    $estadoingreso = $obj->estado;
    $fingreso = $obj->fingreso;

    $fecha_actual= new DateTime($fecha);
    $fecha_entrada= new DateTime($fingreso);

    if($estadoingreso == 'Nuevo'){
